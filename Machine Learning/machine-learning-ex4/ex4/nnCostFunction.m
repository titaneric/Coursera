...
for i=1:m
  ...
  sub_y = zeros(size(a_3));
  sub_y(y(i,:),:) = 1;
  
  cost1 = sub_y .* log(a_3);
  cost0 = (1 .- sub_y) .* log(1 .- a_3);
...
sub_theta1 = Theta1(:,2:columns(Theta1));
sub_theta2 = Theta2(:,2:columns(Theta2));

J += (lambda / (2 * m)) * (sum( (sub_theta1 .^ 2)(:)) + sum( (sub_theta2 .^ 2)(:)));
